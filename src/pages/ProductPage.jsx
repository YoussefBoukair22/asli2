  ...
  if (!loading && !product) {
    return (
      <div className="pt-16 min-h-screen bg-white flex flex-col items-center justify-center gap-5">
        ...
        <button onClick={() => navigate('/shop')} className="bg-black text-white px-8 py-3 text-xs font-bold tracking-widest uppercase hover:opacity-70 transition-opacity">
          Back to Shop
        </button>
      </div>
    )
  ...
  return (
    <div className="pt-16 bg-white min-h-screen">
      <div className="px-6 py-3 border-b border-gray-100 max-w-7xl mx-auto">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-[11px] tracking-widest uppercase text-gray-400 hover:text-black transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 max-w-7xl mx-auto">
        {/* Image */}
        <div className="aspect-[3/4] relative overflow-hidden" style={{ background: product.bgColor || '#f5f5f5' }}>
          <motion.div
          ...
          >
            {product.imageUrl
              ...
              : <span className="font-head text-8xl tracking-widest" style={{ color: 'rgba(0,0,0,0.06)' }}>ASLY</span>
            }
          </motion.div>
          {product.isNew && (
            <span className="absolute top-4 left-4 bg-black text-white text-[9px] font-bold tracking-widest uppercase px-2 py-1">New</span>
          )}
        </div>
        ...
        <div className="p-8 md:p-12 border-l border-gray-100 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <span className="text-[10px] tracking-[0.25em] uppercase text-gray-400 block mb-3">{product.category}</span>
            <h1 className="font-head text-[clamp(2.5rem,5vw,4rem)] leading-none tracking-wide text-black mb-4">{product.name}</h1>
            <p className="text-2xl font-light text-gray-400 mb-7">{product.price?.toFixed(2)} MAD</p>
            <p className="text-sm text-gray-500 leading-[1.9] mb-7 max-w-md">{product.description}</p>

            {product.details && product.details.length > 0 && (
              <ul className="mb-7 flex flex-col gap-2">
                {product.details.map(d => (
                  <li key={d} className="flex items-center gap-3 text-xs text-gray-400">
            ...
            <div className="flex gap-3 flex-wrap mb-7">
              ...
              <button
                onClick={() => { addToCart(product, size); navigate('/checkout') }}
            ...
            <div className="border-t border-gray-100 pt-5 flex flex-col gap-3">
              {['Free shipping on orders over 300 MAD', 'Free returns within 30 days', 'Premium materials, ethically sourced'].map(p => (
      ...
      {related.length > 0 && (
        <section className="border-t border-gray-100 py-16 px-6 max-w-7xl mx-auto">
          <div className="mb-8">
            <span className="text-[10px] tracking-[0.3em] uppercase text-gray-400 block mb-2">You Might Also Like</span>
