    ...
    <>
      <header className={`fixed top-0 left-0 right-0 z-[100] h-16 flex items-center justify-between px-6 transition-all duration-200 bg-white ${scrolled ? 'shadow-sm' : ''} border-b border-gray-100`}>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            className="md:hidden p-2 text-black ml-1"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="fixed inset-0 top-16 bg-white z-[99] flex flex-col p-6"
            initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }}
            transition={{ duration: 0.28, ease: [0.4, 0, 0.2, 1] }}
          >
            {/* Logo in mobile menu */}
            <div className="mb-6">
              <img src="/logo.jpg" alt="ASLI" className="h-12 w-auto object-contain" />
            </div>
            ...
            {NAV_LINKS.map(({ to, label }, i) => (
              ...
              >
                <Link
                  to={to}
                  className="block font-head text-5xl tracking-wider text-black border-b border-gray-100 py-5 hover:opacity-50 transition-opacity"
                >
                  {label}
                </Link>
              </motion.div>
